...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    foreach ($payload['entry'] as $entry) {
        foreach ($entry['changes'] ?? [] as $change) {
            ...
            wLog('Lead recibido: ' . $leadId . ' | ad_id=' . ($value['ad_id'] ?? '-'));
            ...
            $fullName  = $parsedData['full_name']
                      ?? trim(($parsedData['first_name'] ?? '') . ' ' . ($parsedData['last_name'] ?? ''))
                      ?: 'Desconocido';
            ...
            try {
                ...
                wLog('Lead guardado en BD: ' . $leadId . ' | nombre=' . $fullName);
            } catch (PDOException $e) {
                wLog('ERROR BD al guardar lead ' . $leadId . ': ' . $e->getMessage());
            }
        }
    }
...
{
    if (empty($signature) || empty(META_APP_SECRET)) return false;

    // Formato: sha256=XXXXXXX
    if (!str_starts_with($signature, 'sha256=')) return false;

    $received = substr($signature, 7);
    $expected = hash_hmac('sha256', $body, META_APP_SECRET);
