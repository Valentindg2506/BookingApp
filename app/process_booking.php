...
if (empty($fullName))                    $errors[] = 'Nombre requerido';
if (!isValidEmail($email))               $errors[] = 'Email inválido';
if (!isValidPhone($phone))               $errors[] = 'Teléfono inválido';
if (!preg_match('/^\d{4}-\d{2}-\d{2}$/', $selDate)) $errors[] = 'Fecha inválida';
if (!preg_match('/^\d{2}:\d{2}$/', $selTime))        $errors[] = 'Hora inválida';
...
if ($waResult['success']) {
    $pdo->prepare("UPDATE appointments SET confirmation_sent = 1 WHERE id = :id")
